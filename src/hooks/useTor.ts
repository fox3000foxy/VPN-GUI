...
export function useTor(initialIpFetched: string | null): UseTorReturn {
  ...
  const startTor = useCallback(async () => {
    ...
    try {
      ...
      const process = await Neutralino.os.spawnProcess(
        "tor-expert-bundle\\tor.exe -f tor-expert-bundle\\torcc --ExitNodes {" +
      ...
      Neutralino.events.on(
        "spawnedProcess",
        async (evt: { detail: SpawnedProcessEventDetail }) => {
                ...
                if (treated) {
                  if (treated.percent === 100) {
                    await Neutralino.os.execCommand(
                      "powershell -ExecutionPolicy Bypass -File proxy.ps1 -Enable",
                    );
                    setIpLoading(true);
                    ...
                    if (newIp) {
                      ...
                      setIpError(
                        "Unable to fetch your IP after Tor activation",
                      );
                    }
                    setIpLoading(false);
                  }
      ...
      setError(
        `Failed to start Tor: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }, [initialIpFetched]);
  ...
  const stopTor = useCallback(async () => {
    try {
      ...
      await Neutralino.os.execCommand(
        "powershell -ExecutionPolicy Bypass -File proxy.ps1 -Disable",
      );
      setTorRunning(false);
      setTorProcessId(null);
      setIp(initialIpFetched);
      ...
      setError(
        `Failed to stop Tor: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }, [initialIpFetched]);
